import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Component } from '@angular/core';

@Component({
  selector: 'app-trip-path-one',
  templateUrl: './trip-path-one.component.html',
  styleUrls: ['./trip-path-one.component.css'],
})
export class TripPathOneComponent {
  constructor(private http: HttpClient) {}

  apiUrl = 'http://localhost:8002'; // Your API URL

  
  responseFromApi: any;

  callUIMCS() {
    const headers = new HttpHeaders({
      Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..bRB6gIYkzCbdbZu5_V-mkhI2ITEfa8qBhyA15jIAa1E`, // Add the JWT token here
    });

    this.http
      .get(`${this.apiUrl}/context-propagation`, { headers })
      .subscribe((x) => {
        this.responseFromApi = "Success";
        alert('Success')
      });
  }
}
